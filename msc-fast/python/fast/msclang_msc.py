...
def getSizeFromFormat(formatString):
    size = _FORMAT_SIZE_CACHE.get(formatString)
    if size is None:
        size = 0
        for char in formatString:
            size += TYPE_SIZES[char]
        _FORMAT_SIZE_CACHE[formatString] = size
    return size


COMMAND_SIZES = {cmd: getSizeFromFormat(fmt) for cmd, fmt in COMMAND_FORMAT.items()}
COMMAND_UNPACK_FROM = {
    cmd: struct.Struct(ENDIANESS + fmt).unpack_from
    for cmd, fmt in COMMAND_FORMAT.items()
    if fmt
}
COMMAND_PACK = {
    cmd: struct.Struct(ENDIANESS + fmt).pack
    for cmd, fmt in COMMAND_FORMAT.items()
    if fmt
}
...
class Command:
    ...
    def read(self, byteBuffer, pos):
        op = byteBuffer[pos]
        command = op & 0x7F
        self.pushBit = (op & 0x80) != 0
        if command in COMMAND_NAMES:
            self.command = command
            self.paramSize = COMMAND_SIZES[command]
            unpacker = COMMAND_UNPACK_FROM.get(command)
            if unpacker is None:
                self.parameters = []
            else:
                self.parameters = list(unpacker(byteBuffer, pos + 1))
        else:
    ...
    def write(self, endian=ENDIANESS):
        ...
        fmt = COMMAND_FORMAT[command]
        if command in (0xFFFE, 0xFFFF):
            returnBytes = bytes()
        else:
            returnBytes = bytes([command | (0x80 if self.pushBit else 0x0)])
        if endian == ENDIANESS and command not in (0xFFFE, 0xFFFF):
            packer = COMMAND_PACK.get(command)
            if packer is None:
                return returnBytes
            return returnBytes + packer(*(self.parameters[i] & 0xffffffff for i in range(len(fmt))))
        for i, paramChar in enumerate(fmt):
...
class MscScript:
    ...
    def getIndexOfInstruction(self, location):
        cmds = self.cmds
        cache = self._pos_index
        if cache is None or self._pos_index_len != len(cmds):
            cache = {}
            for i, cmd in enumerate(cmds):
                position = getattr(cmd, "commandPosition", None)
                if position is not None and position not in cache:
                    cache[position] = i
            self._pos_index = cache
            self._pos_index_len = len(cmds)
