...
def build_cfg(script):
    cached = getattr(script, "_cfg_blocks", None)
...
def _is_command_like(cmd_obj):
    return getattr(cmd_obj, "command", None) is not None and getattr(cmd_obj, "parameters", None) is not None
