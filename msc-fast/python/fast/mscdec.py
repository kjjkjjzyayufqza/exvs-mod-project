...
_POINTER_MAP_CACHE = {}
_RE_FUNC_NAME_POINTER = re.compile(r'func_name: (\w+), pointer: (\d+)')
_RE_FUNC_241 = re.compile(r'func_241\(\s*0x[0-9a-fA-F]+,\s*(0x[0-9a-fA-F]+)\s*\);')
_RE_FUNC_241_SUB = re.compile(r'(func_241\(\s*0x[0-9a-fA-F]+,\s*)0x[0-9a-fA-F]+(\s*\);)')
_RE_SYS1_DISPATCH = re.compile(r'sys_1\(\s*0x10001\s*,\s*0x10\s*,\s*\w+\s*,\s*(\w+)\s*\(')
_RE_ARG0_EQ = re.compile(r'0x([0-9a-fA-F]+)\s*==\s*arg0')
_RE_VAR1_HEX = re.compile(r'var1 = (0x[0-9a-fA-F]+);')
_RE_VAR1_HEX_STMT = re.compile(r'var1 = 0x[0-9a-fA-F]+;')
_RE_EXTERNAL_ACTION_TABLE = re.compile(r'\w+\s*=\s*sys_0\(\s*0x700000\s*,\s*0\s*,\s*\w+\s*,\s*0xa\s*\)\s*;')
_RE_ACTION_DISPATCHER = re.compile(r'sys_1\(\s*0x10001\s*,\s*0x10\s*,\s*\w+\s*,\s*\w+\s*\(')
BINARY_OPERATIONS = {
    0xe  : "+",
    0xf  : "-",
    0x10 : "*",
...
def main(args):
    ...
    ep_script = mscFile.getScriptAtLocation(mscFile.entryPoint)
    old_ep_name = ep_script.name if ep_script else None
    if ep_script:
        ep_script.name = 'main'
    funcNames = []
        ...
        for script in mscFile.scripts:
            for cmd in script.cmds:
                if isinstance(cmd, Command) and cmd.command in (0x0A, 0x0D):
                    if isinstance(cmd.parameters[0], ScriptRef) and str(cmd.parameters[0]) == old_ep_name:
                        cmd.parameters[0] = ScriptRef('main')
    if args.assumeCharStd:
...
def read_function_pointer_map(log_file):
    ...
    cached = _POINTER_MAP_CACHE.get(log_file)
    if cached is not None:
        return cached
    pointer_map = {}
    with open(log_file, 'r', encoding='utf-8') as log:
        for line in log:
            match = _RE_FUNC_NAME_POINTER.search(line)
            if match:
                pointer_map[int(match.group(2))] = match.group(1)
    _POINTER_MAP_CACHE[log_file] = pointer_map
    return pointer_map
...
def handle_func_241_pointer_funcs(file_name, log_file):
    ...
    for line_index, line in enumerate(content):
        ...
        pointer_match = _RE_FUNC_241.search(line)
        if pointer_match:
            original_line = line
            ...
            if function_name:
                ...
                replaced_str = _RE_FUNC_241_SUB.sub(
...
def handle_sys_1_0x10001_0x10_var1_pointer_funcs(file_name, log_file):
    ...
    for line in content:
        line_count += 1
        target_func_match = _RE_SYS1_DISPATCH.search(line)
        ...
        if target_func in line:
            ...
            for current_index in range(line_count - 1, len(content)):
                ...
                if '== arg0' in current_line:
                    condition_match = _RE_ARG0_EQ.search(current_line)
                    if condition_match:
                        ...
                        while assignment_line_index < len(content):
                            ...
                            if 'var1 = 0x' in assignment_line:
                                pointer_match = _RE_VAR1_HEX.search(assignment_line)
                                if pointer_match:
                ...
                if 'var1 = 0x' in current_line:
                    ...
                    pointer = _RE_VAR1_HEX_STMT.search(original_line)
                    if pointer:
                        # find the pointer value and + 0x30 and format it remove ";"
                        pointer_value = _RE_VAR1_HEX.search(original_line).group(1)
                        pointer_value_int = int(pointer_value, 16) + 0x30
                        function_name = pointer_map.get(pointer_value_int)
                        replaced_str = None
                        if function_name:
...
def is_new_external_action_msc(file_name):
    ...
    has_external_action_table = _RE_EXTERNAL_ACTION_TABLE.search(source)
    has_action_dispatcher = _RE_ACTION_DISPATCHER.search(source)
