use crate::ir::{Arg, Cmd, Item};
use crate::opcode as op;
use crate::parse::{Expr, Function, Stmt, Unit};
        ...
        match e {
            ...
            Expr::Ident(name) => {
                if let Some(&idx) = self.local_index.get(name) {
                    self.emit(Cmd::new(op::CMD_PUSH_VAR, vec![Arg::U32(0), Arg::U32(idx as u32)]));
                } else if let Some(&idx) = self.globals.get(name) {
                    self.emit(Cmd::new(op::CMD_PUSH_VAR, vec![Arg::U32(1), Arg::U32(idx as u32)]));
