...
export interface NumdlbNamingOptions {
   ...
   * This resolves to: "E:\\XB\\解包\\com\\file\\0x092B6B4A\\23.numdlb"
   */
  rootDir: string;
  /**
...
export async function applyNumdlbBaseNameToStructureObject(
  ...
  await mapWithConcurrency(candidates, concurrency, async (item) => {
    ...
    const absPath = await join(options.rootDir, ...segments.map((s) => normalizeSeparatorsToBackslash(s)));
  ...
  if (folder010) {
    ...
    for (const e of binItems) {
      const beforeSegments = splitPathSegments(e.fileUrl);
      const absPath = await join(options.rootDir, ...beforeSegments.map((s) => normalizeSeparatorsToBackslash(s)));
      const bytes = await readFile(absPath);
  ...
  for (const e of nutexbItems) {
    ...
    if (fileSize < 0x8) {
      throw new Error(`Invalid nutexb size: ${absPath}`);
    }

    const footerMagic = buf.slice(fileSize - 0x8, fileSize - 0x4).toString("ascii");
    if (footerMagic !== " XET") {
      throw new Error(`Invalid nutexb footer magic: ${absPath}`);
    }
    ...
    if (majorVersion === 1 && minorVersion === 1) {
      ...
      throw new Error(`Unsupported nutexb version: ${majorVersion}.${minorVersion} (${absPath})`);
    }

    if (nameOffset < 0 || nameOffset + 4 > fileSize) {
      throw new Error(`Invalid nutexb name offset: ${absPath}`);
    }

    const prefix = buf.slice(nameOffset, nameOffset + 4).toString("ascii");
    if (prefix !== "46XT") {
      throw new Error(`Invalid nutexb format: missing 46XT prefix: ${absPath}`);
    }
