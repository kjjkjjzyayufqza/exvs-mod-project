...
export function DaeExportDialog({
  ...
  const resolvedSubtitle =
    subtitle ?? (targets.length === 1 ? (targets[0]?.name ?? t("dae.exportTarget")) : t("dae.objectsSelected", { count: targets.length }));
  ...
  const chooseOutputDirectory = async () => {
    const selected = await openDialog({
      directory: true,
      title: t("dae.selectExportFolder"),
    ...
    >
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">
            ...
            <>
              {ssbhCount > 0 && (
                <p className="text-muted-foreground">
                  {t("dae.ssbhModel", { count: ssbhCount })}
                </p>
              )}
              {daeCount > 0 && (
                <p className="text-muted-foreground">
                  {t("dae.importedMesh", { count: daeCount })}
                </p>
              )}
            </>
          )}
        ...
        <div className="space-y-1.5">
          <Label className="text-xs">{t("dae.outputDirectory")}</Label>
          <div className="flex gap-2">
            <Input
              value={outputDirectory}
              onChange={(e) => setOutputDirectory(e.target.value)}
              className="h-8 text-xs"
              placeholder={t("dae.chooseExportFolder")}
            />
            <Button
              type="button"
              variant="outline"
              ...
              aria-label={t("dae.chooseExportFolder")}
            >
        ...
        {showFormatSelector ? (
          ...
          <div className="space-y-1.5">
            <Label className="text-xs">{t("dae.format")}</Label>
        ...
        <div className={`grid gap-3 ${supportsDae ? "grid-cols-2" : "grid-cols-1"}`}>
          {supportsDae ? (
            <div className="space-y-1.5">
              <Label className="text-xs">{t("dae.scaleFactor")}</Label>
              <Input
                type="number"
                min={0.001}
                step={0.1}
          ...
          <div className="space-y-1.5">
            <Label className="text-xs">{t("dae.upAxis")}</Label>
            <Select value={upAxis} onValueChange={(v) => setUpAxis(v as "y_up" | "z_up")}>
              <SelectTrigger className="h-8 text-xs">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="y_up">{t("dae.yUp")}</SelectItem>
                <SelectItem value="z_up">{t("dae.zUp")}</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
        ...
        {targets.length > 0 && (
          <div className="flex items-center gap-2">
            ...
            <Label htmlFor="export-textures" className="text-xs cursor-pointer">
              {t("dae.exportReferencedTextures")}
            </Label>
          </div>
        )}
      </div>
