...
export function EffectProjectEditorModalWindow({
  ...
  const { nodeRef, handleProps } = useDraggableModal({
    defaultPosition: { x: 32 + cascadeIndex * 24, y: 32 + cascadeIndex * 24 },
  });
  ...
  }, [runSaveWithLatestDraft, nodeRef]);
  ...
  return (
    <div ref={zLayerRef} className="pointer-events-none absolute inset-0" aria-hidden={false}>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby={`effect-project-editor-title-${session.id}`}
        tabIndex={-1}
        className="pointer-events-auto w-[min(920px,96vw)] max-w-[96vw]"
        style={{ position: "absolute" }}
        onClick={(e) => e.stopPropagation()}
      >
        <Card className="flex min-h-0 max-h-[min(92vh,820px)] flex-col overflow-hidden border shadow-2xl">
          <div
            {...handleProps}
            onPointerDown={(e) => {
              onActivate();
          ...
          >
            <div className="flex min-w-0 flex-1 items-center gap-3">
              ...
              <div className="min-w-0">
                <h2
                  id={`effect-project-editor-title-${session.id}`}
                ...
                <p className="text-xs text-muted-foreground">Effect project (.bin)</p>
              </div>
            </div>
            <Button
              type="button"
