...
import { DepictionPropertyPanel } from "./DepictionPropertyPanel";
import { ProjectilePreview3D } from "./ProjectilePreview3D";
import { useDepictionEditorStore } from "./DepictionEditorStore";
...
const STORE_KEY = "paramEditors.v2.fp.projectile_depiction_table";
const PARAM_TYPE = "projectile_depiction_table";
...
export function DepictionEditorView({
  ...
  const data = useDepictionEditorStore((s) => s.data);
  const selectedIndex = useDepictionEditorStore((s) => s.selectedIndex);
  const dirty = useDepictionEditorStore((s) => s.dirty);
  const validationMessages = useDepictionEditorStore(
    (s) => s.validationMessages,
  );

  const store = useDepictionEditorStore;
  ...
  const renderEntryLabel = (row: EditorEntryRow) => (
    <div className="flex flex-col">
      ...
      <span className="text-[9px] text-muted-foreground">#{row.index}</span>
    </div>
  );
    ...
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-[220px_minmax(0,1fr)_320px]">
        ...
        <div className="min-h-0 border-x">
          {data ? (
            <ProjectilePreview3D entry={entry} />
          ) : (
            <div className="flex h-full items-center justify-center text-sm text-muted-foreground">
              Select a projectile depiction table file to begin
            </div>
          )}
        </div>
