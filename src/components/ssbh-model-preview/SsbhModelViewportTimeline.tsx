...
export function SsbhModelViewportTimeline({
  ...
  /** Bone inspector is heavy and easy to mis-click during playback — collapsed by default. */
  const [boneInspectorOpen, setBoneInspectorOpen] = useState(false);
  const [rangeToolsOpen, setRangeToolsOpen] = useState(false);
  ...
  /** Remember play state across scrub so drag/seek does not permanently cancel playback. */
  ...
  const beginTimelineScrub = (clientX: number): void => {
    ...
    // Pause only for the duration of the drag; resume if we were playing.
    wasPlayingBeforeScrubRef.current = p.motionPlaying;
    if (p.motionPlaying) {
      p.setMotionPlaying(false);
    }
  ...
  /** Frame step / explicit stop — intentional pause, not a scrub side-effect. */
  const seekFrameAndPause = (frame: number): void => {
    wasPlayingBeforeScrubRef.current = false;
    const next = clampFrame(frame, activeMin, activeMax);
    playheadRef.current = next;
    setScrubFrame(null);
  ...
  return (
    <div className="rounded-md border border-border/60 bg-background/90 px-2 py-1.5 backdrop-blur-sm">
      {/* Transport cluster: intentional play controls only */}
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        >
          <Button
            ...
            onClick={() => {
              ...
              // Keep play state (video-player style jump-to-start).
        ...
        <div className="flex items-center gap-1 rounded-md border border-border/40 px-1.5 py-0.5">
          {([0.25, 0.5, 1, 1.5, 2] as const).map((v) => (
            <Button
              ...
              type="button"
              size="sm"
              variant={Math.abs(p.motionSpeed - v) < 1e-6 ? "secondary" : "ghost"}
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2.5 pr-0.5">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Switch
              id="viewport-loop"
              checked={p.motionLoop}
              onCheckedChange={p.setMotionLoop}
              disabled={!hasMotion}
            />
          </div>
        </div>
      ...
      <div className="mt-1.5 rounded border border-border/50 bg-background/70 p-1.5">
        <div
          ...
          onPointerDown={(e) => {
            ...
            // Only start scrub from the track itself — ignore accidental clicks on nested UI.
        ...
        >
          ...
          <div className="relative h-7 bg-background/40">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1">
            ...
            <Input
              ...
              onBlur={() => {
                ...
                // Explicit frame entry pauses — intentional seek for inspection.
                seekFrameAndPause(Math.round(parsed));
                setFrameInputText(String(Math.round(clampFrame(Math.round(parsed), activeMin, activeMax))));
              }}
            />
          </div>
          <Button
          ...
          <div className="ml-auto flex items-center gap-1">
            <Button
              type="button"
              size="sm"
              variant={rangeToolsOpen || rangeEnabled ? "secondary" : "ghost"}
            ...
            <Button
              type="button"
              size="sm"
              variant={boneInspectorOpen ? "secondary" : "ghost"}
              className="h-6 px-2 text-[10px]"
      ...
      {rangeToolsOpen ? (
        <div className="mt-1.5 flex flex-wrap items-center gap-1.5 rounded-md border border-border/40 bg-muted/10 px-2 py-1.5">
          <Button
            type="button"
            size="sm"
            variant="outline"
          ...
          <div className="ml-auto flex items-center gap-2 pr-1">
            ...
            />
            <span className="font-mono text-[10px] tabular-nums text-muted-foreground">
              [{Math.min(rangeIn, rangeOut).toFixed(0)} – {Math.max(rangeIn, rangeOut).toFixed(0)}]
            </span>
          </div>
        </div>
      ) : null}
        ...
        <div className="mt-1.5 rounded-md border border-border/50 bg-background/70 p-2">
          ...
          <div className="grid grid-cols-1 gap-2 md:grid-cols-[220px_minmax(0,1fr)]">
            <div className="space-y-2">
              ...
              />
              <div ref={boneRowsRef} className="h-[140px] overflow-y-auto rounded border border-border/50 p-1">
