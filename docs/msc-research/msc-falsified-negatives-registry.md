...
| I12 | Rebellion roll-sub 677 **每一 tick** `func_296(0x3e8, 0)` | **SHOOT 全程自由落体**，形态闩仍是鸟，落地后解除变形（假鸟态） | 禁止再关 `sys_1(0x30001)`。Hambrabi 这条招不碰电机；鸟主射也不碰。用户 2026-09-01 澄清可见问题是**持续按后会往后飞、镜头跟着偏**，不是锁朝向写入。下一刀只把 `global454`（`func_594` 在 `0x1000000` 时的 leftover scale）改成鸟主射的 0，不要清 `0x4000`，不要抄足止 clamp / `func_167` | [hambrabi-flight-sub-side-roll-shot](./hambrabi-flight-sub-side-roll-shot.md) §4.10；2026-09-01 user |
