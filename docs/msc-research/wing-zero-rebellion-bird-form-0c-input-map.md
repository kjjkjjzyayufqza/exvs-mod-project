...
| Input bit | TV 语义（摘要） | Rebellion 语义 |
|-----------|-----------------|----------------|
| **`0x1`** | **idle / 待机类** | **主射** |
| **`0x100`** | **主射**（鸟形态换成飞行主射 hash） | **特射** |
| `0x80` | 鸟形态有独立飞行侧映射 | 副射 |
| `0x200` | 副射/特格侧（鸟有第二套 hash） | 特格 |
...
## 4. Rebellion 鸟形态 `func_143` 当前策略（2026-08-14）

尚无完整 TV 级“第二套飞行武器 hash / 资源表 swap”时：

| 策略 | 内容 |
|------|------|
| 普通形态 `global39 == 0` | 保留完整 `func_95` 表 |
| 鸟形态 `global39 != 0` | **不**映射主射/副射/特射/特格/近战/蓄力 bit |
| 可选保留 | partner/`0x400` + `0xc0000` 条件那条（与 TV 鸟分支类似） |
...
| 鸟 `global143==0x2` | 主射不可发动；无 bird 武器 hash 时副射/特射/近战也不应从 thinker 进入 |
...
| ID | 项 | 状态 |
|----|----|------|
| O1 | Port TV 鸟形态专用主射/副射 hash 到 Rebellion | 未做 |
