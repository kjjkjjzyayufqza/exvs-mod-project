...
## 4.11 **解决（E3 确认 2026-09-01）：yaw 是对的，查询接口用错了**

**用户实机结论：「不管按什么后都不会影响锁定了」。** Stage 1–13 全部为 E3-，
根因是 SHOOT 逐帧锁敌用错了目标查询，不是 analog、mix、电机或 profile。

**这条订正 §4.10 结尾的判断** ——「mix/analog 才是 I10 之后的合法下一刀，
yaw 不是」是错的。yaw 就是那一刀，只是 I10 那版写成了：

```c
sys_46(0, func_102(sys_0(0x40000, 0x5), 0x5, 0x1));      // E3- 完全无效
```

**正确写法**（`sub_shot_flight_roll_tick`，`global184 == 0x2` 分支）：

```c
if (global39 != 0xffffffff && sys_0(0x70003, global39))
{
    sys_46(0, func_102(sys_0(0x40000, 0x3, global39), 0x1f4, 0x2));
    func_105(func_102(func_609(0x1) - global268, 0x1f4, 0x2), 0, 0);
}
```
...
| | 原版 | I10 那版 |
|---|---|---|
| 目标查询 | `sys_0(0x40000, **0x3**, global39)` 显式锁定目标 | `sys_0(0x40000, **0x5**)` |
| 有效性守卫 | `global39 != 0xffffffff && sys_0(0x70003, global39)` | 无 |
| 限速器 | `func_102(diff, 0x1f4, **0x2**)` 每帧角速度上限 | `func_102(diff, 0x5, **0x1**)` 每帧比例 |

**`sys_0(0x40000, 0x5)` 不是逐帧锁敌接口。** 全语料里它只出现在 driver 内部
（`func_626` / `func_601` / `func_595`）和 `0.c func_97` 的回退分支。

**两处出处，互相印证：**

- `002zgundm_001zgundm_001 func_1011` —— Ζ，可变形机，飞行动作相位体里逐帧锁敌：
  `global267 = func_102(sys_0(0x40000, 0x3, global39), 0x1f4, 0x2); sys_46(0, global267);`
- 本机 `rebellion_sub_shot_custom_aim` —— 地面副射，已实证，同形，速率 `0x3e8`，
  水平 `sys_46(0)` 与垂直 `func_105` 成对。

**为什么必须在 SHOOT 写：** `func_595` 是这个动作整个调用闭包（85 个函数）里
**唯一**的朝向写入者，且只在 `global184 == 0x1` 期间执行。SHOOT + END 那段
没有任何 MSC 朝向写入，所以航向被原生所有者接管。

**语料量化**（538 台机体）：相位体与 `callFunc3` tick 里共 7566 处 `sys_46(0, …)`，
其中 7243 处是 `sys_46(0, VAR)`，VAR 由上一行 `func_102(...)` 算出。
**逐帧写朝向是原版常规做法（25.4% 的 tick 会写），不是异常。**

### 同轮撤销的三项（均 E3- 或无先例）

| 项 | 处置 |
|---|---|
| `func_296(0x3e9, 0)` + `func_313(0x64)` 解耦移动所有者（Stage 8 的正式实装版） | 实机无变化，已撤销 |
| `func_351` profile 切换（START `0x1` / SHOOT `0` / END `0x2`，Stage 13） | Hambrabi 飞行态一次都不调（`func_878` 被 `global24 & 0x2` 挡着）。已全删；registry D10 的成因随之消失 |
| SHOOT 每帧 `func_296(0x3e8, 0)`（Stage 6 / I12 飞行马达） | 已删，见 §4.11 下方 |

### 同轮其它已确认修正

| 项 | 内容 |
|---|---|
| **伪飞行模式**（E3） | SHOOT 每帧关飞行马达 → 整个突发自由落体、形态锁还在。Hambrabi 整条路径不碰马达（他唯一的 `func_296` 是 `0x3e9`，走 `sys_46(0x6)`） |
| **时钟统一** | 三段改为 clip 时钟：START `func_309(global20, 0x8 * 0x64)`；SHOOT `func_309(global20, sys_47(0x1, sys_4B(0x1)) - 0x1f4) \|\| func_91()`（`func_972` 原式）；END `func_91()`（`func_974`）。START 首帧 `func_89(0x2fd, 0)` 选中 `func_91` 的 `sys_47(0x7)` clip-finished 分支 |
| **ENTER 合并** | `_L` / `_R` 合成 `SUB_SHOT_FLIGHT_ROLL`，侧别按 `global87 & 0x20` 在函数内选。合并前两份已分叉：左边丢了 `global681 = 0x1` 和 `global693 = 0`，一直跑在弹槽 0 |
| **tick 清理** | 按键读取搬到 `677` / `679`。语料：26625 个 tick 体里只有 **11 个**（0.04%）读按键；但 **25.4%** 写 `sys_46(0, …)`，所以瞄准留在 tick 是对的 |
| **渐进转向** | tick 里每帧一次性快照会盖掉 `func_595` 的窗口渐进，表现为「瞬间对准」。俯仰改用 `func_595` 每帧算好的 `global499` + `global715` |

---

## 4.12 **垂直对准 = 骨骼瞄准注册（E3 确认 2026-09-01）**

**症状：** 敌人在斜下方，机体保持水平，弹道从敌人上方过去。水平对准正常。

**根因：`func_144` 的整个函数体在 `if (global115 > 0)` 里面，而我们从未注册过。**

链路：

```text
func_594（START）
  → func_71(global676)        动作自己的 START 体先跑
  → func_616(global716)       再应用
      → func_144(global716, 0, mode)
          if (global115 > 0)  ← global115 只有 func_142 / func_615 会加
              sys_47(0x16, global20, <bone>, ..., 0x64, ...)   骨骼瞄准目标
              sys_47(0x20, global20, <bone>, global322 * 0x64, global323 * 0x64, ...)
                                                              用 ±60 / -65 限位
```

`sys_47(0x16)` / `sys_47(0x20)` 是**一次性设置、引擎持续维护**的骨骼瞄准约束，
用的正是 `func_609` 里那对 `global322` / `global323` 俯仰限位。
**所以 Hambrabi 全程不需要任何逐帧垂直代码** —— 这也是之前所有尝试都"毫无变化"
的原因：在一条没启用的机制旁边写代码。

**修法（一行）：** START 首帧

```c
func_615(0x1, 0xe, 0x8, 0);      // = func_142(0x1, 0xe, 0); global716 = 0x8;
```

参数出处：Hambrabi 的側転 hook `func_1082` / `func_1085` 设
`global951 = 0x1; global952 = 0xe;`，`func_959` 用
`func_615(global951, global952, global840, 0)` 注册（`global840` = row 字段 0x1d
= START 时长，我们是 8f）。
**本机先例：`special_shot_flight_start`（飞行特射，同为鸟形态）
`func_615(0x1, 0x1, 0x14, 0)`。**

`global115` 只在 `func_386`（初始化/复活）复位，`func_142` 上限 8 条 ——
这是"每条命注册一次"的列表，和本机其它调 `func_615` 的动作行为一致。

### 七次无效尝试，全部因为机制未启用

| 尝试 | 实机结果 |
|---|---|
| `func_105(step, 0, 0)` 累加姿态 | 无变化 |
| `func_104(绝对角, 0, 0)` 赋值姿态 | 无变化 |
| 俯仰塞进 `sys_46(0x1, 0x2, …)` 的 Y 槽 | 无变化 |
| `sys_46(0x1, 0x1, 0, V, T)` 垂直通道 | **机体往下平移，仍水平** —— 证明那是平移不是旋转 |
| `func_300(0)` → `func_301(0)` 保住垂直缩放 | 无变化 |
| 姿态层 B `func_107(0, 0, 0)` 归零 | 无变化 |
| `sys_4C(0x8, 0x3)` + `global183 = 0` | 无变化 |
| **诊断：** clip 换成鸟 loop `0x9de587ce` | **仍水平 → clip 不是原因** |

### 同轮钉死的两条负面事实

1. **`func_601` 的俯仰分支在所有机体上都是死代码。** 门是 `if (global720)`，
   而 `func_600` 在三条路径上全部 `global720 = 0`。所以
   **Hambrabi 的垂直也不来自 `func_601`**，为垂直去换 `func_599` driver 没有意义。
   （`func_599` 确实能直接驱动我们现有四槽，`func_600/601/602/603` 与 Hambrabi
   逐字相同，但那是另一回事。）
2. `global47 |= 0x40`（TV 与 Bound Doc 的 tick 都有）是 `func_283` 的**时间缩放**
   标志：置位后跳过 `sys_0(0x60010)` / `sys_0(0x60011)`，让动作跑满速。
   不是瞄准开关。

### 方法论

这条是**全闭包 syscall 差集**扫出来的（Hambrabi 153 函数 / 84 种 syscall+子命令，
