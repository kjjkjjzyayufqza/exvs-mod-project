 ...
 * Run: node scripts/generate-hex-template2.js
 */

import fs from 'fs'
import path from 'path'

const OUTPUT_BIN = 'E:\\XB\\解包\\com\\file\\0xA014012A\\test2.bin'

const TEMPLATE_HEX = `
00 00 00 00 00 00 00 00 04 00 00 00 F5 7F 84 0C 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 A0 40 00 00 00 00 00 00 00 00 00 00 00 00 A0 00 00 00 00 00 00 00 00 00 00 00 00 00 B4 42 00 00 34 42 00 00 00 00 B0 0E 3A 36 00 00 00 00 00 00 00 00 00 00 0C 42 00 00 00 00 00 00 F0 41 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 01 00 00 00 00 00 00 00 00 00 00 00 64 2A 8B 73 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 E0 40 00 00 00 00 00 00 00 00 00 00 00 00 00 00 48 42 F6 5D 0F FC 00 00 34 42 00 00 00 00 00 00 00 00 00 00 00 00 00 00 40 40 00 00 00 00 00 00 00 00 00 00 00 00 92 5A 9A FF 00 00 00 00 00 00 20 41 00 00 00 00 EF 01 60 92 00 00 F0 41 00 00 00 00 00 00 00 00 13 07 EC F8 CD CC 4C BE 00 00 00 00 00 00 00 00 17 E6 F0 7F 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 E0 40
`
...
/**
 * When true and PLACEMENTS_PER_BONE length matches BONE_INDEX_HEX, only left_right is taken
 * from the array; up_down is always from UP_DOWN (resolveRangedFloat). LEFT_RIGHT is ignored when overrides apply.
 */
const USE_PLACEMENT_OVERRIDES = true

/**
 * Per-record left_right overrides (paste from plot-lr-ud.html "Copy all result").
 * up_down fields are ignored in this script — up_down comes from UP_DOWN per record index.
...
function main () {
  ...
  if (USE_PLACEMENT_OVERRIDES && PLACEMENTS_PER_BONE && PLACEMENTS_PER_BONE.length === BONE_INDEX_HEX.length) {
    console.log('left_right: PLACEMENTS_PER_BONE (overrides); up_down: UP_DOWN per record (PLACEMENTS_PER_BONE.up_down ignored)')
  }
