...
fn main() {
    ...
    println!("final_frame={} groups={}", anim.final_frame_index, anim.groups.len());
    for g in &anim.groups {
        if !matches!(g.group_type, GroupType::Transform) { continue; }
        for n in &g.nodes {
            for t in &n.tracks {
                let TrackValues::Transform(vals) = &t.values else { continue };
                ...
                let flag = if max_dt < 1e-4 && max_dr < 1e-4 { "STATIC" } else { "MOVING" };
                println!(
