...
fn main() {
    ...
    for (i,b) in skel.bones.iter().enumerate() {
        let parent = b.parent_index.map(|p| format!("{}({})", skel.bones[p].name, p)).unwrap_or_else(|| "-".into());
        if b.name.contains("KOSHI") || b.name.contains("MUNE") || b.name=="BASE" || b.name.contains("SAKOTSU") || b.name.starts_with("ATH_IP_KATA") || b.name.starts_with("ATH_OR") {
