...
mod tests {
    ...
    #[test]
    fn matrix_maps_columns_like_dae_import_bone() {
        let m = Matrix {
            m00: 1.0,
            m10: 0.0,
            m20: 0.0,
        ...
        let t = matrix_3x4_to_import_columns(&m);
        assert_eq!(t[3][0], 5.0);
        assert_eq!(t[3][1], 6.0);
        assert_eq!(t[3][2], 7.0);
        assert_eq!(t[3][3], 1.0);
    ...
    /// Loads a real FBX that may use empty mesh element names (ufbx reports `element.name` as "").
    /// Default path is the author's fixture; override with `SSBH_TEST_FBX_WEIGHTS`.
    ///
    /// Run: `cargo test parse_fbx_weights_fixture_smoke -- --ignored --nocapture`
    #[test]
    #[ignore = "Requires local FBX on disk; see SSBH_TEST_FBX_WEIGHTS or default path in test body"]
    fn parse_fbx_weights_fixture_smoke() {
        let path_str = std::env::var("SSBH_TEST_FBX_WEIGHTS").unwrap_or_else(|_| {
            r"E:\XB\解包\com\file\test\完成权重.fbx".to_string()
        });
        let fbx = Path::new(&path_str);
        assert!(
            fbx.is_file(),
            "FBX not found at {}. Set SSBH_TEST_FBX_WEIGHTS to an existing file or place the fixture.",
            fbx.display()
        );

        let scene = parse_fbx_file(fbx).unwrap_or_else(|e| {
            panic!("parse_fbx_file failed for {}: {:#}", fbx.display(), e);
        });

        assert!(
            !scene.meshes.is_empty(),
            "expected at least one mesh in {}",
            fbx.display()
        );
        for m in &scene.meshes {
            assert!(
                !m.name.is_empty(),
                "imported mesh names must be non-empty (fallback naming)"
            );
            assert!(
                !m.vertices.is_empty(),
