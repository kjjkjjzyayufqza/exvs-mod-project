...
fn main() {
    let path = std::env::args().nth(1).unwrap_or_else(|| r"E:\XB\解包\com\test\test.fhm2d".to_string());
    ...
    for f in &extraction.files {
        if f.file_type == ".bin" {
            // Try to identify
            let id = if f.data.windows(8).any(|w| w.eq_ignore_ascii_case(b"vdk_type")) {
                "placement.csv"
            } else if f.data.windows(20).any(|w| w.eq_ignore_ascii_case(b"directional_lighting")) {
                "graphic_param.csv"
            ...
            eprintln!("  file_index={} type={} size={} identified={}", f.file_index, f.file_type, f.data.len(), id);
        }
    }

    let (tree, warnings) = stage_rename_in_memory_numatb_based(&extraction.files, &extraction.sub_file_structure).unwrap();
...
fn render(folder: &StageVirtualTreeFolder, prefix: &str, is_root: bool) -> String {
    ...
    if is_root { out.push_str(&format!("{}/\n", folder.name)); }
