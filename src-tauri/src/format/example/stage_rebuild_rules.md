...
**Source**: `unk2_for_ext()` at `fhm2d_stage.rs:3031`
...
in `push_folder()` at `fhm2d_stage.rs:3246`.
...
**Source**: `file_type_for_ext()` at `fhm2d_stage.rs:3023`
...
occurrence's `fileUrl` is used. This is handled by `add_nutexb_dedup()` at
`fhm2d_stage.rs:3219`.
...
**Source**: SSBH detection and ordering in `emit_dir_recursive_inner()` at
`fhm2d_stage.rs:3376`.
...
**Source**: `stage_content_dir_order()` at `fhm2d_stage.rs:3296`.
...
**Source**: `is_texture_container_dir()` at `fhm2d_stage.rs:2997`.
...
identical to N separate `EndMark(1)` entries. Verified in
`serialize_structure_binary` at `fhm2d_pack.rs:623-627`.
...
### 6.2 Structure Comparison (Manual)
...
8. **Modifying push_folder without updating serialize_structure_binary**: The
   binary serializer at `fhm2d_pack.rs:582-631` writes unk5 at a specific offset.
   Any new Folder fields must be reflected in both the JSON emission and binary
   serialization.
...
|----------|-----------|---------|
| `is_texture_container_dir` | `fhm2d_stage.rs:2997` | Detect texture container dirs |
| `file_type_for_ext` | `fhm2d_stage.rs:3023` | Map extension to SubFileData.fileType |
| `unk2_for_ext` | `fhm2d_stage.rs:3031` | Map extension to Item.unk2 type tag |
| `add_nutexb_dedup` | `fhm2d_stage.rs:3219` | Texture deduplication by filename |
| `push_folder` | `fhm2d_stage.rs:3246` | Emit Folder node (auto-sets unk5 for unk3=32) |
| `push_item` / `push_item_ex` | `fhm2d_stage.rs:3229/3233` | Emit Item node |
| `emit_texture_container_items` | `fhm2d_stage.rs:3307` | Emit tex container folder + items + EndMark |
| `emit_file_item` | `fhm2d_stage.rs:3335` | Emit single file as Item with correct unk2 |
| `emit_dir_recursive_inner` | `fhm2d_stage.rs:3376` | Main tree walker (SSBH detection + ordering) |
| `stage_content_dir_order` | `fhm2d_stage.rs:3296` | Content-level dir sort order |
| `rebuild_structure_from_scratch` | `fhm2d_stage.rs:3615` | Entry point (per-model textures) |
| `rebuild_structure_from_scratch_with_shared_textures` | `fhm2d_stage.rs:3691` | Entry point (shared textures mode) |
| `serialize_structure_binary` | `fhm2d_pack.rs:582` | Binary serialization of structure tree |
| `parse_sub_file_structure` | `fhm2d.rs:650` | Binary parser (reference for field offsets) |
